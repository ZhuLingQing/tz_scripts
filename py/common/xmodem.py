...
class XMODEM(object):
    ...
    def send(self, stream, retry=16, timeout=60, quiet=0):
        ...
        while True:
            ...
            if char:
                if char == NAK:
                    ...
                    log.error('send ERROR expected NAK/CRC, got `%s`' % (char,))
        ...
        # end of transmission, resend EOT for increased reliability
        self.putc(EOT)
        char = self.getc(1, timeout)
        if char == NAK:
            self.putc(EOT)
            ...
            if char == ACK:
                ...
                log.error('expect ACK as end of transmission')
        else:
            log.error('expect NAK to resend EOT')
    ...
    def calc_crc(self, data, crc=0):
        ...
        '''
        for char in data:
            crc = (crc << 8) ^ self.crctable[((crc >> 8) ^ char) & 0xff]
        return crc & 0xffff
